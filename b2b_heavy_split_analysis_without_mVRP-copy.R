...
source("/home/shibaprasad/b2b_heavy_split/capacitated_clustering_tsp_tripCosts.R")
...
GetTripCosts <- function(data,
  ...
  n_centers <- max(ceiling(sum(lm_selected_sc$demand, na.rm=T)/(30*lowest_large_cluster_demand)),1)
...
RunProductAnalysis <- function(data, cnid_df, product_type) {
  ...
  for (cnid_in in cnid_list) {
    ...
    Sys.umask(002)
    fname = str_glue(PATH_OUTPUT,"split_analysis_trip_details",toString(product_type),"_", ".RData")
